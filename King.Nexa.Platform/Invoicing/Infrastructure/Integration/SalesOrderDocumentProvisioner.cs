    ...
    INotificationRecordCommandService notifications) : IOrderDocumentProvisioner
{
    ...
    public async Task ProvisionAcceptedOrderAsync(Order order, int clientAccountId, CancellationToken cancellationToken = default)
    {
        ...
        await paymentProcesses.CreateAsync(new PaymentProcessRecord
        {
            ...
            Discount = 0,
            Shipping = 0,
            Igv = Math.Round(order.Total.Amount * 0.18m, 2),
            Total = Math.Round(order.Total.Amount * 1.18m, 2),
            Status = "pending"
        }, cancellationToken);
