    ...
    IUnitOfWork unitOfWork) : ISeedDataService
{
    ...
    private async Task SeedClientAccountsAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var record in records)
        {
            ...
            {
                existingClient.UpdateFrom(clientAccount);
                continue;
            }
        ...
        foreach (var record in records)
        {
            ...
            if (existingUser is not null)
            {
                if (existingUser.Role != record.Role || existingUser.Email != record.Email)
                {
                    ...
                    replacement.UpdateProfile(record.FullName, string.Empty, "en", true);
                         ...
                         existingUser.FullName.Equals(existingUser.Email, StringComparison.OrdinalIgnoreCase))
                {
                    var preferredLanguage = existingUser.PreferredLanguage is "en" or "es"
                        ...
                        : "en";
                    existingUser.UpdateProfile(record.FullName, existingUser.Phone, preferredLanguage, existingUser.CriticalNotificationsEnabled);
            ...
            else
            {
                ...
                user.UpdateProfile(record.FullName, string.Empty, "en", true);
