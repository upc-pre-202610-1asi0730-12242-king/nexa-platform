...
{
    [HttpGet] public async Task<ActionResult<IEnumerable<WorkspaceResource>>> GetAll(CancellationToken ct) => Ok((await queries.ListWorkspacesAsync(ct)).Select(x => TenantAdministrationResourceAssembler.ToResource(x)));
    ...
    [HttpGet("by-slug/{slug}")] public async Task<ActionResult<WorkspaceResource>> GetBySlug(string slug, CancellationToken ct) { var x = await queries.FindWorkspaceBySlugAsync(slug, ct); return x is null ? NotFound() : Ok(TenantAdministrationResourceAssembler.ToResource(x)); }
    [HttpPost, Authorize(Policy = NexaAuthorizationPolicies.CanManageWorkspace)] public async Task<ActionResult<WorkspaceResource>> Create(UpsertWorkspaceResource r, CancellationToken ct) { try { var x = await commands.CreateWorkspaceAsync(TenantAdministrationResourceAssembler.ToEntity(r), ct); return CreatedAtAction(nameof(GetById), new { id = x.Id }, TenantAdministrationResourceAssembler.ToResource(x)); } catch (InvalidOperationException e) { return BadRequest(new { message = e.Message }); } }
